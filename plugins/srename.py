#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# a plugin by @yameteee_yamete_kudasai
...
from pyrogram import Client as YameteeeBot
...
@YameteeeBot.on_message(pyrogram.filters.command(["scaption"]))
...
@YameteeeBot.on_message(pyrogram.filters.command(["srename"]))
async def srename_doc(bot, update):
    update_channel = Config.UPDATE_CHANNEL
    cmd = update.text.split(" ", 1)[0]
    if update_channel:
        try:
    ...
    try:
       caption_text2 = caption_text.caption.format(filename = the_real_download_location)
    except:
